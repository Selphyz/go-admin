...
func CreateOrder(c *fiber.Ctx) error {
	...
	for _, requestedProduct := range request.Products {
		...
		product.Id = uint(requestedProduct["product_id"])
		...
		item := models.OrderItem{
			...
			Price:             product.Price,
			Quantity:          uint(requestedProduct["quantity"]),
		...
		lineItems = append(lineItems, &stripe.CheckoutSessionLineItemParams{
			...
			Quantity:    stripe.Int64(int64(requestedProduct["quantity"])),
		})
	}
	stripe.Key = "sk_test_51GxXUaIW5y5JlfaDuZ3vJ6vrJxNGoVsVYdng2poJk18mT2Bg92UgcD4H7wJ4AIrzPVLmPdjgO6QKX5vptivujWAl00sn5FZ3L5"
...
func CompleteOrder(c *fiber.Ctx) error {
	...
	go func(order models.Order) {
		...
		smtp.SendMail("host.docker.internal:1025", nil, "no-reply@mail.com", []string{order.AmbassadorEmail}, ambassadorMessage)
		smtp.SendMail("host.docker.internal:1025", nil, "no-reply@mail.com", []string{"admin@admin.com"}, adminMessage)
	}(order)
	return c.JSON(fiber.Map{
		"message": "sucess",
	})
